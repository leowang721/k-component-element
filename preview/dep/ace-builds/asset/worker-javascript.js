            ...
            function (_dereq_, module, exports) {
                ...
                var JSHINT = function () {
                        ...
                        var api, bang = {
                                ...
                                '/': true,
                                '%': true
                            }, declared, exported, functionicity = [
                                'closure',
                                'exception',
                                'global',
                                'label',
                                ...
                                'unused',
                                'var'
                            ], funct, functions, global, implied, inblock, indent, lookahead, lex, member, membersOnly, predefined, scope, stack, unuseds, urls, extraModules = [], emitter = new events.EventEmitter();
                            ...
                            if (_.has(funct, name) && !funct['(global)']) {
                                if (funct[name] === true) {
                                    if (state.option.latedef) {
                                        if (state.option.latedef === true && _.contains([
                                                funct[name],
                                                type
                                            ], 'unction') || !_.contains([
                                                funct[name],
                                    ...
                                    if ((!state.option.shadow || _.contains([
                                            'inner',
                                            'outer'
                                        ], state.option.shadow)) && type !== 'exception' || funct['(blockscope)'].getlabel(name)) {
                        ...
                        function expression(rbp, initial) {
                            ...
                            var isDangerous = state.option.asi && state.tokens.prev.line !== startLine(state.tokens.curr) && _.contains([
                                    ']',
                                    ')'
                                ], state.tokens.prev.id) && _.contains([
                                    '[',
                                    '('
                        ...
                        reservevar('this', function (x) {
                            if (state.directive['use strict'] && !isMethod() && !state.option.validthis && (funct['(statement)'] && funct['(name)'].charAt(0) > 'Z' || funct['(global)'])) {
                                warning('W040', x);
                            }
                        });
                        reservevar('true');
                        ...
                        prefix('(', function () {
                            ...
                            if (exprs.length > 1) {
                                ...
                                if (!isNecessary) {
                                    isNecessary = opening.beginsStmt && (ret.id === '{' || triggerFnExpr || isFunctor(ret)) || ret.id === '{' && preceeding.id === '=>' || first.id === '+' && preceeding.id === '+';
                                }
                            }
                            if (ret) {
                                        ...
                                        if (tokens.hasOwnProperty(t)) {
                                            ...
                                            if (funct['(global)']) {
                                                if (predefined[t.id] === false) {
                                                    ...
                                                    if (!state.option.inES5() && vars.ecmaIdentifiers[5][t.id] === false || !state.option.inESNext() && vars.ecmaIdentifiers[6][t.id] === false) {
