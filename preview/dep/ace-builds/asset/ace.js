...
define('ace/layer/marker', [
    ...
    (function () {
        ...
        this.drawTextMarker = function (stringBuilder, range, clazz, layerConfig, extraStyle) {
            var row = range.start.row;
            var session = this.session;
            var lineRange = new Range(row, range.start.column, row, session.getScreenLastRowColumn(row));
        ...
        this.onChangeTabSize = this.$computeTabString = function () {
            ...
            for (var i = 1; i < tabSize + 1; i++) {
                if (this.showInvisibles) {
                    tabStr.push('<span class=\'ace_invisible ace_invisible_tab\'>' + this.TAB_CHAR + lang.stringRepeat(' ', i - 1) + '</span>');
                } else {
